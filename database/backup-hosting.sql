...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-06-2023 a las 02:09:13
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_books_lookfor` (IN `_type` CHAR(1) CHARSET utf8mb4, IN `_look` VARCHAR(150) CHARSET utf8mb4)   BEGIN
			if _type = "n" then
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_books_register` (IN `_idcategorie` INT, IN `_idsubcategorie` INT, IN `_amount` VARCHAR(30), IN `_descriptions` VARCHAR(150), IN `_author` VARCHAR(150), IN `_state` VARCHAR(30), IN `_locationresponsible` VARCHAR(50), IN `_url` VARCHAR(150), IN `_frontpage` VARCHAR(150))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_books_update` (IN `_idbook` INT, IN `_idcategorie` INT, IN `_idsubcategorie` INT, IN `_amount` VARCHAR(30), IN `_descriptions` VARCHAR(150), IN `_author` VARCHAR(150), IN `_state` VARCHAR(30), IN `_locationresponsible` VARCHAR(50))   BEGIN
			UPDATE books SET
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_edit_categorie` (IN `_idcategorie` INT, IN `_categoryname` VARCHAR(50))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_edit_subcategorie` (IN `_idcategorie` INT, IN `_idsubcategorie` INT, IN `_subcategoryname` VARCHAR(50))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_list_dashboard_books` ()   SELECT 	COUNT(idbook) AS total_libros , 
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_list_dashboard_users` ()   SELECT  COUNT(idusers) AS total_usuarios,
						(SELECT COUNT(*) FROM users WHERE accesslevel LIKE 'D') AS total_docentes 
					FROM users
				END$$

CREATE DEFINER=CURRENT_USER PROCEDURE `spu_loans_list` ()   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_loans_register` (IN `_idbook` INT, IN `_idusers` INT, IN `_observation` VARCHAR(100), IN `_loan_date` DATETIME, IN `_return_date` DATETIME, IN `_amount` VARCHAR(30))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_register_categorie` (IN `_categoryname` VARCHAR(50))   BEGIN
					INSERT INTO categories(categoryname)
					VALUES(_categoryname);
			END$$

CREATE DEFINER=CURRENT_USER PROCEDURE `spu_register_commentaries` (IN `_idbook` INT, IN `_idusers` INT, IN `_commentary` VARCHAR(250), IN `_score` INT)   BEGIN
					INSERT INTO commentaries(idbook,idusers,commentary,score)
					VALUES(_idbook,_idusers,_commentary,_score);
			END$$

CREATE DEFINER=CURRENT_USER PROCEDURE `spu_register_subcategorie` (IN `_idcategorie` INT, IN `_subcategoryname` VARCHAR(50))   BEGIN
					INSERT INTO subcategories(idcategorie, subcategoryname)
					VALUES(_idcategorie,_subcategoryname);
			END$$

CREATE DEFINER=CURRENT_USER PROCEDURE `spu_searchuser` (IN `_username` VARCHAR(150))   BEGIN
	  SELECT 
		idusers,
		username,
		surnames,
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_users_login` (IN `_email` VARCHAR(100))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_users_register` (IN `_username` VARCHAR(50), IN `_surnames` VARCHAR(30), IN `_namess` VARCHAR(30), IN `_email` VARCHAR(100), IN `_accesskey` VARCHAR(100), IN `_accesslevel` VARCHAR(100))   BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_users_update` (IN `_idusers` INT, IN `_namess` VARCHAR(30), IN `_surnames` VARCHAR(100), IN `_email` VARCHAR(100), IN `_accesslevel` VARCHAR(100), IN `_accesskey` VARCHAR(100))   BEGIN
			UPDATE users SET
				namess 		= _namess,
				surnames 	= _surnames,
				email 		= _email,
...
CREATE DEFINER=CURRENT_USER PROCEDURE `spu_validate_email` (IN `_email` VARCHAR(100))   BEGIN
					SELECT email 
						FROM users
					WHERE email = _email;
			END$$
...
--
-- Estructura de tabla para la tabla `bookschinchanos`
--

CREATE TABLE `bookschinchanos` (
  `idbookchinchano` int(11) NOT NULL,
  `descriptions` varchar(150) NOT NULL,
...
--
-- Estructura de tabla para la tabla `recuperarclave`
--
...
--
-- Volcado de datos para la tabla `recuperarclave`
--
...
ALTER TABLE `books`
  ...
  ADD KEY `fk_idcategorie_categories` (`idcategorie`),
  ADD KEY `fk_idsubcategorie_subcategories` (`idsubcategorie`);

--
-- Indices de la tabla `bookschinchanos`
--
ALTER TABLE `bookschinchanos`
  ADD PRIMARY KEY (`idbookchinchano`);
...
ALTER TABLE `loans`
  ...
  ADD KEY `fk_idbook_idbook` (`idbook`),
  ADD KEY `fk_idusers_idusers` (`idusers`);

--
-- Indices de la tabla `recuperarclave`
--
ALTER TABLE `recuperarclave`
  ADD PRIMARY KEY (`idrecuperar`),
  ADD KEY `fk_idusuario_rcl` (`idusuario`);
...
ALTER TABLE `subcategories`
  ...
  ADD KEY `fk_idcategorie_subcategories` (`idcategorie`);
...
ALTER TABLE `users`
  ...
  ADD UNIQUE KEY `ul_email_usu` (`email`),
  ADD UNIQUE KEY `email` (`email`),
  ADD UNIQUE KEY `uk_user_names` (`username`);
...
--
ALTER TABLE `books`
  MODIFY `idbook` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=353;

--
-- AUTO_INCREMENT de la tabla `bookschinchanos`
--
ALTER TABLE `bookschinchanos`
  MODIFY `idbookchinchano` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `commentaries`
  MODIFY `idcommentary` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;

--
-- AUTO_INCREMENT de la tabla `loans`
--
ALTER TABLE `loans`
  MODIFY `idloan` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT de la tabla `recuperarclave`
--
ALTER TABLE `recuperarclave`
...
--
ALTER TABLE `subcategories`
  MODIFY `idsubcategorie` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
...
--
ALTER TABLE `books`
  ADD CONSTRAINT `fk_idcategorie_categories` FOREIGN KEY (`idcategorie`) REFERENCES `categories` (`idcategorie`),
  ADD CONSTRAINT `fk_idsubcategorie_subcategories` FOREIGN KEY (`idsubcategorie`) REFERENCES `subcategories` (`idsubcategorie`);
...
ALTER TABLE `commentaries`
  ...
  ADD CONSTRAINT `fk_idusers` FOREIGN KEY (`idusers`) REFERENCES `users` (`idusers`);

--
-- Filtros para la tabla `loans`
--
ALTER TABLE `loans`
  ADD CONSTRAINT `fk_idbook_idbook` FOREIGN KEY (`idbook`) REFERENCES `books` (`idbook`),
  ADD CONSTRAINT `fk_idusers_idusers` FOREIGN KEY (`idusers`) REFERENCES `users` (`idusers`);

--
-- Filtros para la tabla `recuperarclave`
--
ALTER TABLE `recuperarclave`
  ADD CONSTRAINT `fk_idusuario_rcl` FOREIGN KEY (`idusuario`) REFERENCES `users` (`idusers`);

--
-- Filtros para la tabla `subcategories`
--
ALTER TABLE `subcategories`
  ADD CONSTRAINT `fk_idcategorie_subcategories` FOREIGN KEY (`idcategorie`) REFERENCES `categories` (`idcategorie`);
COMMIT;
